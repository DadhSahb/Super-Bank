...
const loadForEdit = () => {
  let html_format = stored_products.map((item) => {
    //Q5: assigning the row a class name "some"
    let rowClass = "";
  ...
  document.getElementById("content_holder").innerHTML = `<table class="table">
      <thead>
        <tr>
          <th scope="col">Id</th>
      ...
      <tbody>
        <tr>${html_format.reduce((acc, format) => acc + format)}</tr>
      </tbody>
    </table>`;
};
