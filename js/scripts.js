...
let peanut = new Product(6, "Peanuts", 14.05, "/assets/peanuts.jpg");
...
let products = [
  ...
  peanut,
  new Product(7, "Deodorant", 6.99, "/assets/products/deodorant.jpg"),
  new Product(
    8,
    "Mechanical Drill",
    122.75,
    "/assets/products/mechanical_drill.jpg"
  ),
];
...
function authenticateMember(member) {
  ...
  let members = JSON.parse(localStorage.getItem("members"));
  if (members) {
    let logged_member = members.find(
      (m) => m.email == member.email && m.password == member.password
    );
    if (logged_member) {
